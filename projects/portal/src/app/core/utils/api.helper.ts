...
export const httpGet = <RType>(url: string, showLoader: boolean = true): Observable<RType> => {
    return getHttpClient().get<RType>(apiBaseUrl + url, { headers: getHeaderWithLoaderConfigure(showLoader) });
};

export const httpPost = <RType, PType>(url: string, payload: PType, showLoader: boolean = true): Observable<RType> => {
    return getHttpClient().post<RType>(apiBaseUrl + url, payload, { headers: getHeaderWithLoaderConfigure(showLoader) });
};

export const httpPut = <RType, PType>(url: string, payload: PType, showLoader: boolean = true): Observable<RType> => {
    return getHttpClient().put<RType>(apiBaseUrl + url, payload, { headers: getHeaderWithLoaderConfigure(showLoader) });
};

export const httpDelete = <T>(url: string, showLoader: boolean = true): Observable<T> => {
    return getHttpClient().delete<T>(apiBaseUrl + url, { headers: getHeaderWithLoaderConfigure(showLoader) });
};
